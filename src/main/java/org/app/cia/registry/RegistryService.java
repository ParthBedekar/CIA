...
public class RegistryService {
    private final Map<String,RepoRegistry> repoRegistryMap;
    ...
    public RepoRegistry getOrRegister(String url, Path base){
        if(!repoRegistryMap.containsKey(url)){
            ...
            repoRegistryMap.put(url, new RepoRegistry(repoPath, snapshots.get(0), snapshots.get(1), hashes.get(0), hashes.get(1), changedFiles));           }else{
            manager=new GitManager(url,base);
        ...
        return repoRegistryMap.get(url);
    }
}
